...
def _run_real_training(task_id: str) -> None:
    ...
    try:
        ...
        write_log(task_id, f"数据集：{dataset.name}（{dataset.sample_count} 样本）")
        ...
        result_model = AIModel(
            ...
            status="ready",
            description=f"基于 {base_model.name} 在数据集「{dataset.name}」上真实 LoRA 微调（{epochs} epochs, r={lora_r}, loss={final_loss:.3f}）",
...
def _run_mock_training(task_id: str) -> None:
    ...
    try:
        ...
        stages = [
            ...
            (20, 44, "▶ 阶段 2/4：数据预处理", [
                f"读取数据集，共 {dataset.sample_count} 样本", "文本清洗与归一化...", "分词处理...", "数据预处理完成"]),
            (44, 90, "▶ 阶段 3/4：模型训练", []),
        ...
        result_model = AIModel(
            ...
            status="ready",
            description=f"（模拟）基于 {base_model.name} 微调",
            base_model_id=base_model.id,
        )
        db.add(result_model)
        db.flush()
